...
func MapToStruct(Map map[string]interface{}, StructPointer interface{}, converter Converter, tag string) {
	...
	for i := 0; i < structType.NumField(); i++ {
		...
		case reflect.Slice:
			m1 := make([]map[string]interface{}, len(v.([]interface{})))
			for index, i := range v.([]interface{}) {
				m1[index] = i.(map[string]interface{})
			}
			sliceType := f.Type()
			structType := sliceType.Elem()
			slice := reflect.MakeSlice(sliceType, 0, len(m1))
			for _, m := range m1 {
				s := reflect.New(structType)
				MapToStruct(m, s.Interface(), converter, tag)
				slice = reflect.Append(slice, s.Elem())
			}
			// reflect.ValueOf(f).Elem().Set(slice)
			f.Set(slice)
		default:
			// not implemented
		}
